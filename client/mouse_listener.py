...
class MouseListener(BaseListener):
    ...
    def __init__(self, session_id):
        super().__init__(session_id, mouse.Listener(on_move=self.on_move, on_scroll=self.on_scroll))
        self.motion_list = []
        self.scroll_list = []
    ...
    def on_move(self, x, y):
        ...
        self.motion_list.append([self.session_id, time_moved, x, y])

    def on_scroll(self, x, y, dx, dy):
        time_scrolled = int(time.time())
        rotation = -1 if dy < 0 else 1
        self.scroll_list.append([self.session_id, time_scrolled, x, y, rotation])
